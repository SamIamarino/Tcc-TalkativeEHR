...
// Atualiza a tela quando a Alexa acionar um quarto
// ============================================================
...
// ------------------------------------------------------------
// Consulta /status a cada 3s — detecta qual quarto a Alexa ativou
// ------------------------------------------------------------
setInterval(async () => {
  try {
...
// ------------------------------------------------------------
// Busca dados do paciente — retorna o paciente_id para buscar evoluções
// ------------------------------------------------------------
async function buscarPaciente(quartoId) {
  try {
    ...
    return p.id; // retorna o id para buscar evoluções
...
// ------------------------------------------------------------
// Busca as evoluções do paciente e renderiza os cards
// ------------------------------------------------------------
...
function limparTela() {
  const ids = [
    "paciente-nome", "paciente-idade", "paciente-sexo", "paciente-sangue",
    "paciente-alergias", "paciente-internado", "paciente-contato-emergencia",
    "paciente-telefone-emergencia", "leito-numero", "leito-andar", "leito-ala",
  ...
  const container = document.getElementById("evolucoes-container");
  if (container) {
    container.innerHTML = `<p class="content-text" style="opacity:0.5;padding:1rem;">Nenhuma evolução carregada.</p>`;
  }
}
